...
public class AssetRepository : IAssetRepository
{
    ...
    private static T[] Query<T>(string dbPath, string sql, Func<DbDataReader, T> map)
    {
    ...
    {
        const string sql = "SELECT id, object_id, asset_bundle, serialized_file, type, name, game_object, size, pretty_size, crc32, legacy, events FROM animation_view";
        return Query(dbPath, sql, static reader => new Animation
        {
    ...
    {
        const string sql = "SELECT id, asset_name, asset_bundle, type, dep_id, dep_asset_bundle, dep_name, dep_type FROM asset_dependencies_view";
        return Query(dbPath, sql, static reader => new AssetDependencies
        {
            Id = reader.GetInt64(reader.GetOrdinal("id")),
        ...
        }).OrderBy(dep => dep.Id).ToArray();
    }

    public Asset[] GetAssets(string dbPath)
    {
        const string sql = "SELECT asset_name, id, object_id, asset_bundle, serialized_file, type, name, game_object, size, pretty_size, crc32 FROM asset_view";
        return Query(dbPath, sql, static reader => new Asset
        {
            AssetName = reader.GetString(reader.GetOrdinal("asset_name")),
            Id = reader.GetInt64(reader.GetOrdinal("id")),
            ObjectId = reader.GetInt64(reader.GetOrdinal("object_id")),
            ...
            PrettySize = reader.GetValue(reader.GetOrdinal("pretty_size")).ToString() ?? "0B",
            Crc32 = reader.GetInt64(reader.GetOrdinal("crc32"))
        }).OrderBy(asset => asset.Id).ToArray();
    }

    public AudioClip[] GetAudioClips(string dbPath)
    {
        const string sql = "SELECT id, object_id, asset_bundle, serialized_file, type, name, game_object, size, pretty_size, crc32, bits_per_sample, frequency, channels, load_type, format FROM audio_clip_view";
        return Query(dbPath, sql, static reader => new AudioClip
        {
            Id = reader.GetInt64(reader.GetOrdinal("id")),
            ObjectId = reader.GetInt64(reader.GetOrdinal("object_id")),
            AssetBundle = reader.GetString(reader.GetOrdinal("asset_bundle")),
        ...
        }).OrderBy(clip => clip.Id).ToArray();
    }

    public Mesh[] GetMeshes(string dbPath)
    {
        const string sql = "SELECT id, object_id, asset_bundle, serialized_file, type, name, game_object, size, pretty_size, crc32, sub_meshes, blend_shapes, bones, indices, vertices, compression, rw_enabled, vertex_size, channels FROM mesh_view";
        return Query(dbPath, sql, static reader => new Mesh
        {
            Id = reader.GetInt64(reader.GetOrdinal("id")),
            ObjectId = reader.GetInt64(reader.GetOrdinal("object_id")),
            AssetBundle = reader.GetString(reader.GetOrdinal("asset_bundle")),
            ...
            Channels = reader.GetValue(reader.GetOrdinal("channels")).ToString() ?? string.Empty,
        }).OrderBy(mesh => mesh.Id).ToArray();
    }

    public Object[] GetObjects(string dbPath)
    {
        const string sql = "SELECT id, object_id, asset_bundle, serialized_file, type, name, game_object, size, pretty_size, crc32 FROM object_view";
        return Query(dbPath, sql, static reader => new Object
        {
            Id = reader.GetInt64(reader.GetOrdinal("id")),
            ObjectId = reader.GetInt64(reader.GetOrdinal("object_id")),
            AssetBundle = reader.GetString(reader.GetOrdinal("asset_bundle")),
            ...
            PrettySize = reader.GetValue(reader.GetOrdinal("pretty_size")).ToString() ?? "0B",
            Crc32 = reader.GetInt64(reader.GetOrdinal("crc32"))
        }).OrderBy(obj => obj.Id).ToArray();
    }

    public ShaderKeywordRatio[] GetShaderKeywordRatios(string dbPath)
    {
        const string sql = "SELECT shader_id, name, sub_shader, hw_tier, pass, api, pass_name, shader_type, total_variants, keyword, variants, ratio FROM shader_keyword_ratios";
        return Query(dbPath, sql, static reader => new ShaderKeywordRatio
        {
        ...
        }).OrderBy(r => r.ShaderId).ToArray();
    }

    public ShaderSubprogram[] GetShaderSubprograms(string dbPath)
    {
        const string sql = "SELECT shader_id, name, sub_shader, hw_tier, api, pass, pass_name, shader_type, sub_program, keywords FROM shader_subprogram_view";
        return Query(dbPath, sql, static reader => new ShaderSubprogram
        {
            ShaderId = reader.GetInt64(reader.GetOrdinal("shader_id")),
            Name = reader.GetString(reader.GetOrdinal("name")),
            SubShader = reader.GetInt64(reader.GetOrdinal("sub_shader")),
        ...
        }).OrderBy(sp => sp.ShaderId).ToArray();
    }

    public Shader[] GetShaders(string dbPath)
    {
        const string sql = "SELECT id, object_id, asset_bundle, serialized_file, type, name, game_object, size, pretty_size, crc32, decompressed_size, sub_shaders, sub_programs, unique_programs, keywords FROM shader_view";
        return Query(dbPath, sql, static reader => new Shader
        {
            Id = reader.GetInt64(reader.GetOrdinal("id")),
            ObjectId = reader.GetInt64(reader.GetOrdinal("object_id")),
            AssetBundle = reader.GetString(reader.GetOrdinal("asset_bundle")),
            ...
            Keywords = reader.GetValue(reader.GetOrdinal("keywords")).ToString() ?? ""
        }).OrderBy(shader => shader.Id).ToArray();
    }

    public Texture[] GetTextures(string dbPath)
    {
        const string sql = "SELECT id, object_id, asset_bundle, serialized_file, type, name, game_object, size, pretty_size, crc32, width, height, format, mip_count, rw_enabled FROM texture_view";
        return Query(dbPath, sql, static reader => new Texture
        {
            Id = reader.GetInt64(reader.GetOrdinal("id")),
            ObjectId = reader.GetInt64(reader.GetOrdinal("object_id")),
            AssetBundle = reader.GetString(reader.GetOrdinal("asset_bundle")),
            ...
            RwEnabled = reader.GetInt64(reader.GetOrdinal("rw_enabled"))
        }).OrderBy(texture => texture.Id).ToArray();
    }

    public BreakdownByType[] GetBreakdownByType(string dbPath)
    {
        const string sql = "SELECT type, count, byte_size, pretty_size FROM view_breakdown_by_type";
        return Query(dbPath, sql, static reader => new BreakdownByType
        {
            Type = reader.GetString(reader.GetOrdinal("type")),
            Count = reader.GetValue(reader.GetOrdinal("count")).ToString() ?? "0",
            ByteSize = reader.GetValue(reader.GetOrdinal("byte_size")).ToString() ?? "0",
            PrettySize = reader.GetValue(reader.GetOrdinal("pretty_size")).ToString() ?? "0B"
        }).OrderBy(b => b.Type).ToArray();
    }

    public BreakdownShaders[] GetBreakdownShaders(string dbPath)
    {
        const string sql = "SELECT name, instances, pretty_total_size, total_size, in_bundles FROM view_breakdown_shaders";
        return Query(dbPath, sql, static reader => new BreakdownShaders
        {
            Name = reader.GetString(reader.GetOrdinal("name")),
    ...
    {
        const string sql = "SELECT material_id, material_name, material_path, material_asset_bundle, shader_id, shader_name, shader_asset_bundle FROM view_material_shader_refs";
        return Query(dbPath, sql, static reader => new MaterialShaderRefs
        {
        ...
        }).OrderBy(m => m.MaterialId).ToArray();
    }

    public MaterialTextureRefs[] GetMaterialTextureRefs(string dbPath)
    {
        const string sql = "SELECT material_id, material_name, material_path, material_asset_bundle, texture_id, texture_name, texture_asset_bundle FROM view_material_texture_refs";
        return Query(dbPath, sql, static reader => new MaterialTextureRefs
        {
            MaterialId = reader.GetInt64(reader.GetOrdinal("material_id")),
            MaterialName = reader.GetString(reader.GetOrdinal("material_name")),
            MaterialPath = reader.GetString(reader.GetOrdinal("material_path")),
    ...
    {
        const string sql = "SELECT instances, name, type, pretty_total_size, total_size, size, pretty_size, in_files FROM view_potential_duplicates";
    ...
    {
        return Query(dbPath, query, static reader =>
        {
            ...
            for (var i = 0; i < reader.FieldCount; i++)
            {
                ...
                builder.Append($"\"{reader.GetName(i)}\" : \"{reader.GetValue(i)}\"");
            }
            builder.Append(" }");
